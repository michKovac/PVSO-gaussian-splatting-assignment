...
% Šípka a výsledok
\node[font=\small, pvsoGray] at (2.75,-0.80) {$\Downarrow$\enspace triangulácia\enspace$\Rightarrow$\enspace sparse 3D body};

% Sparse cloud – kompaktne
\foreach \x/\y in {1.35/-1.50, 1.85/-1.25, 2.35/-1.55,
                   2.80/-1.28, 3.25/-1.48, 2.05/-1.72, 2.90/-1.70, 2.55/-1.38} {
...
Reálne objektívy \textbf{skresľujú} obraz (radiálna distorzia).
3DGS potrebuje \textbf{ideálny pinhole model}:
\begin{itemize}\setlength{\itemsep}{2pt}
...
\vspace{0.12cm}
\begin{alertblock}{Porovnanie s NeRF}
{\small\setlength{\tabcolsep}{4pt}
\begin{tabular}{lcc}
\toprule
& \textbf{NeRF} & \textbf{3DGS} \\
\midrule
Reprezentácia  & implicitná (MLP)& explicitná \\
Renderovanie   & minúty          & \textbf{real-time} \\
Editovateľnosť & ťažká           & jednoduchšia \\
\bottomrule
\end{tabular}}
\end{alertblock}
...
\draw[pvsoBlue, thick, fill=pvsoBlue!15, rotate around={40:(0,0)}]
    (0,0) ellipse (1.8 and 0.72);
\node[circle, fill=pvsoRed, inner sep=3.5pt] at (0,0) {};
\draw[pvsoGray, dashed, thick] (-1.55,0.45) -- (1.55,-0.45);
\draw[pvsoGray, dashed, thick] (-0.58,-0.72) -- (0.58,0.72);
\draw[-{Stealth}, pvsoRed, thick] (0,0) -- (1.0,0.75)
    node[above right, font=\small]{$\boldsymbol{\mu}$};
\node[font=\small, pvsoBlue, anchor=west] at (2.4, 0.75) {$\alpha = 0.8$};
\node[font=\small, pvsoBlue, anchor=west] at (2.4, 0.25) {farba $= f(\text{smer})$};
\node[font=\small, pvsoBlue, anchor=west] at (2.4,-0.25) {$\boldsymbol{\Sigma}$ = elipsoid};
\end{tikzpicture}
\end{center}
...
\node[font=\footnotesize, pvsoGray, right=0.2cm of dens]
    {pridaj Gaussiány tam kde chýbajú};
\node[font=\footnotesize, pvsoGray, right=0.2cm of optim]
...
\foreach \a in {0,45,...,315} {
    \draw[pvsoRed, -{Stealth[length=4.5pt]}, thick]
        ({3.2*cos(\a)},{1.0*sin(\a)}) -- ({2.94*cos(\a)},{0.92*sin(\a)});
    \draw[pvsoRed, fill=pvsoRed!25, thick]
        ({3.2*cos(\a)},{1.0*sin(\a)})
        -- ({3.2*cos(\a)+0.20*cos(\a-90)},{1.0*sin(\a)+0.20*sin(\a-90)})
        -- ({3.2*cos(\a)+0.20*cos(\a+90)},{1.0*sin(\a)+0.20*sin(\a+90)}) -- cycle;
}
